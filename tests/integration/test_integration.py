...
async def test_relate_and_wait_for_active_status(
    ...
    assert ops_test.model
    await ops_test.model.integrate(relation1=f"{APP_NAME}:fiveg_n2", relation2=AMF_CHARM_NAME)
    await ops_test.model.integrate(relation1=f"{APP_NAME}:fiveg_core_gnb", relation2=NMS_CHARM_NAME)
    await ops_test.model.wait_for_idle(
        apps=[APP_NAME],
...
@pytest.mark.abort_on_fail
async def test_remove_amf_and_wait_for_blocked_status(
    ops_test: OpsTest, deploy_charm_under_test, deploy_dependencies
):
    assert ops_test.model
...
@pytest.mark.abort_on_fail
async def test_restore_amf_and_wait_for_active_status(
    ops_test: OpsTest, deploy_charm_under_test, deploy_dependencies
):
    assert ops_test.model
...
async def deploy_charm_under_test(ops_test: OpsTest, request):
    """Deploy oai-ran-cu-k8s operator."""
    assert ops_test.model
...
async def deploy_dependencies(ops_test: OpsTest):
    """Deploy oai-ran-cu-k8s dependencies."""
    assert ops_test.model
