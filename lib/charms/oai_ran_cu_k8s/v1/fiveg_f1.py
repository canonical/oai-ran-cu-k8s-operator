...
```shell
charmcraft fetch-lib charms.oai_ran_cu_k8s.v1.fiveg_f1
```
...
from charms.oai_ran_cu_k8s.v1.fiveg_f1 import F1Provides
...
from charms.oai_ran_cu_k8s.v1.fiveg_f1 import FivegF1ProviderAvailableEvent, F1Requires
...
class F1Provides(Object):
    ...
    def set_f1_information(
        ...
        if not provider_data_is_valid(
            {"f1_ip_address": ip_address, "f1_port": port, "tac": tac, "plmns": plmns}
        ):
...
class F1Requires(Object):
    ...
    def tac(self) -> Optional[int]:
        ...
        """
        if remote_app_relation_data := self._get_remote_app_relation_data():
            return cast(Optional[int], remote_app_relation_data.get("tac"))
        return None
    ...
    def plmns(self) -> Optional[list[PLMNConfig]]:
        ...
        """
        if remote_app_relation_data := self._get_remote_app_relation_data():
            return cast(Optional[list[PLMNConfig]], remote_app_relation_data.get("plmns"))
        return None
